...
// Team overall rating (average merit score) for each of the last `days` UTC
// days. Each day is scored from its own last snapshot, using only the history
// available up to that point, so past days don't drift as new data arrives.
// Also tracks per-player daily merit ranks to surface who sat in the bottom
// `bottomN` most often across the window (the "bottom watch").
function calcTeamRatingHistory(snapshots, days = 7, bottomN = 5) {
  if (!snapshots || snapshots.length === 0) return { history: [], overall: null, bottomWatch: [] };
  const latestDay = new Date(snapshots[snapshots.length - 1].timestamp).toISOString().slice(0, 10);
  const history = [];
  const perPlayer = {};
  for (let i = days - 1; i >= 0; i--) {
    const d = new Date(new Date(latestDay + 'T00:00:00Z').getTime() - i * 86400000);
    const dayIso = d.toISOString().slice(0, 10);
    const upTo = snapshots.filter(s => new Date(s.timestamp).toISOString().slice(0, 10) <= dayIso);
    const daySnap = upTo[upTo.length - 1];
    if (!daySnap || new Date(daySnap.timestamp).toISOString().slice(0, 10) !== dayIso) {
      history.push({ date: dayIso, rating: null });
      continue;
    }
    const analysed = analyseAllPlayers(daySnap.players, upTo);
    const avg = analysed.reduce((s, p) => s + p.meritScore, 0) / analysed.length;
    history.push({ date: dayIso, rating: Math.round(avg) });
    const total = analysed.length;
    for (const p of analysed) {
      const k = _nk(p.name);
      if (!perPlayer[k]) perPlayer[k] = { name: p.name, ranks: [], daysInBottom: 0 };
      const pp = perPlayer[k];
      pp.ranks.push(p.meritRank);
      if (p.meritRank > total - bottomN) pp.daysInBottom++;
      pp.currentRank = p.meritRank;
      pp.currentRating = p.meritScore;
      pp.totalPlayers = total;
    }
  }
  const vals = history.map(h => h.rating).filter(v => v != null);
  const overall = vals.length ? Math.round(vals.reduce((a, b) => a + b, 0) / vals.length) : null;
  const daysWithData = vals.length;
  const bottomWatch = Object.values(perPlayer)
    .filter(p => p.daysInBottom > 0)
    .map(p => ({
      name: p.name,
      daysInBottom: p.daysInBottom,
      daysTracked: p.ranks.length,
      avgRank: Math.round(p.ranks.reduce((a, b) => a + b, 0) / p.ranks.length),
      currentRank: p.currentRank,
      currentRating: p.currentRating,
      totalPlayers: p.totalPlayers,
    }))
    .sort((a, b) => b.daysInBottom - a.daysInBottom || b.avgRank - a.avgRank)
    .slice(0, 10);
  return { history, overall, daysWithData, bottomWatch };
}
...
module.exports = { _nk, analyseAllPlayers, calcMostImproved, calcImprovementBetween, calcTeamRatingHistory };
