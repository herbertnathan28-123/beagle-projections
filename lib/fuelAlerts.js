...
// Compact money for the total spend of a buy (qty × price ÷ 1000).
const fmtMoney = n => { n = Math.round(n); return n >= 1e9 ? '$' + (n/1e9).toFixed(2) + 'B' : n >= 1e6 ? '$' + (n/1e6).toFixed(1) + 'M' : '$' + n.toLocaleString('en-US'); };
...
function planAmountAt(plan, targetDay, slot) {
  ...
  // prices repeat daily — index by slot (0..47)
  const fprice = Array.isArray(plan.fprices) && slot < plan.fprices.length ? Number(plan.fprices[slot]) || 0 : 0;
  const cprice = Array.isArray(plan.cprices) && slot < plan.cprices.length ? Number(plan.cprices[slot]) || 0 : 0;
  return { fuel: fuel > 0 ? fuel : 0, co2: co2 > 0 ? co2 : 0, fprice, cprice };
}
...
function buildAlertMessage(discordId, label, type, qty, lead, price) {
  const head = '<@' + discordId + '> ⏰ **' + (type === 'fuel' ? 'FUEL' : 'CO2') + ' buy ' + (lead || 'in 5 min') + '** · ' + label;
  // Price the game is charging at this slot, plus the resulting spend — the two
  // numbers a player actually needs to act. Bold + $ so it reads as a price.
  const priceStr = price && price > 0 ? ' @ 💰 **$' + Number(price).toLocaleString('en-US') + '**/1k' : '';
  const costStr = price && price > 0 ? ' · spend ' + fmtMoney(qty * price / 1000) : '';
  const line = type === 'fuel'
    ? '⛽ Fuel: buy **' + fmt(qty) + '** Lbs' + priceStr + costStr
    : '🌿 CO2: buy **' + fmt(qty) + '** quotas' + priceStr + costStr;
  return head + '\n' + line;
}

// Split a slot's amounts into per-type entries (fuel first, then CO2).
function splitByType(base, fuel, co2, fprice, cprice) {
  const out = [];
  if (fuel > 0) out.push({ ...base, type: 'fuel', qty: fuel, price: fprice || 0 });
  if (co2  > 0) out.push({ ...base, type: 'co2',  qty: co2,  price: cprice || 0 });
  return out;
}
...
function imminentBuys(plan, now) {
  ...
  out.push(...splitByType({ day: now.getUTCDate(), slot: curSlot, label: slotLabel(curSlot), minsLeft: 0 }, cur.fuel, cur.co2, cur.fprice, cur.cprice));
  ...
  if (minsToNext <= 5) {                         // > 15 → the :15/:45 warning still handles it
    ...
    out.push(...splitByType({ day: t.getUTCDate(), slot: nSlot, label: slotLabel(nSlot), minsLeft: minsToNext }, nx.fuel, nx.co2, nx.fprice, nx.cprice));
  }
  return out;
}
...
function dueAlerts(plans, now, maxAgeDays) {
  ...
  for (const did of Object.keys(plans || {})) {
    ...
    const { fuel, co2, fprice, cprice } = planAmountAt(plan, target.day, target.slot);
    ...
    for (const t of splitByType({ discordId: did, slot: target.slot, label: target.label }, fuel, co2, fprice, cprice)) {
      out.push({ ...t, message: buildAlertMessage(did, target.label, t.type, t.qty, null, t.price) });
    }
  }
  return out;
}
