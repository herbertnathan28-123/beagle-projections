...
function addSVSnapshot(svHistory, sv, ts) {
  ...
  svHistory.snapshots.push(s);
  // keep last 90 days of snapshots, trimmed to a sensible cap
...
function _normAllianceName(name) { return (name || '').toLowerCase().trim(); }
function addAllianceSVSnapshot(allianceHistory, name, sv, ts) {
  ...
  allianceHistory[key].snapshots.push({ sv, timestamp: ts });
  const cutoff = new Date(Date.now() - 90 * 86400000).toISOString();
  allianceHistory[key].snapshots = allianceHistory[key].snapshots.filter(x => x.timestamp >= cutoff);
...
function recalcAlliancePaces(liveData, allianceHistory) {
  ...
  for (const a of liveData.alliances) {
    ...
    if (pace != null) {
      ...
      a.pace = pace;
    } else if (a.pace == null || isNaN(a.pace) || a.pace < 0.5) {
      console.log('[STARTUP] Preserved/unknown pace for ' + a.name + ': ' + a.pace);
    } else {
      console.log('[STARTUP] Preserved pace for ' + a.name + ': ' + a.pace);
    }
  }
}
...
function loadState() {
  ...
  if (saved) {
    ...
    if (historyPace != null) {
    ...
    } else if (saved.beaglePace == null || isNaN(saved.beaglePace) || saved.beaglePace < 1.0) {
      ...
      console.log('[STARTUP] Preserved saved beaglePace: ' + saved.beaglePace);
    }
...
module.exports = {
  ...
  calcAlliancePaceFromHistory, recalcAlliancePaces,
