...
// Drop any snapshot whose SV is unchanged from the one before it — those are
// stale re-pastes recorded before the addSVSnapshot guard existed, and they
// stretch pace denominators. Keeping the FIRST of each run preserves the real
// capture time.
function pruneStaleDuplicates(snapshots) {
  if (!Array.isArray(snapshots)) return snapshots;
  return snapshots.filter((s, i) => i === 0 || Math.abs(s.sv - snapshots[i - 1].sv) >= 0.005);
}
function loadSVHistory() {
  const h = readJSON(cfg.SV_HISTORY_FILE, { snapshots: [] });
  h.snapshots = pruneStaleDuplicates(h.snapshots);
  return h;
}
...
function addSVSnapshot(svHistory, sv, ts) {
  ...
  // An unchanged SV at a later timestamp is a re-paste of the same game capture,
  // not new data — recording it would stretch pace denominators (stale-duplicate).
  const last = svHistory.snapshots[svHistory.snapshots.length - 1];
  if (existing === -1 && last && Math.abs(last.sv - sv) < 0.005 && ts > last.timestamp) {
    return svHistory;
  }
...
function loadAllianceSVHistory() {
  const h = readJSON(cfg.ALLIANCE_SV_HISTORY_FILE, {});
  for (const key of Object.keys(h)) {
    if (h[key] && Array.isArray(h[key].snapshots)) h[key].snapshots = pruneStaleDuplicates(h[key].snapshots);
  }
  return h;
}
...
function addAllianceSVSnapshot(allianceHistory, name, sv, ts) {
  ...
  // Same stale-duplicate guard as addSVSnapshot: an unchanged SV re-pasted later
  // is the same game capture and must not move the snapshot clock forward.
