...
    --bg: #FFFFFF; --surface: #F0F2F5; --header-bg: #1A2744;
    --border: #AAAAAA; --border-light: #BBBBBB; --text: #000000; --text-dim: #FFFFFF;
    --accent: #1A72BB; --accent-glow: rgba(26,114,187,0.15);
    --dead-bg: #BBBBBB; --dead-text: #000000; --dead-border: #888888;
    --x-bg: #FF8080; --x-text: #000000;
    --t1-bg: #1B5E20; --t1-txt: #FFFFFF;
    --t2-bg: #2E7D32; --t2-txt: #FFFFFF;
    --t3-bg: #43A047; --t3-txt: #FFFFFF;
    --t4-bg: #81C784; --t4-txt: #000000;
    --t5-bg: #C8E6C9; --t5-txt: #000000;
    --neg-bg: #FFCDD2; --neg-txt: #B71C1C;
  }
...
  .control-label { font-size: 10px; font-weight: 700; letter-spacing: 0.12em; text-transform: uppercase; color: #333333; }
...
  .mode-btn { padding: 6px 16px; background: #D0D5E0; border: none; color: #000000; font-size: 12px; font-weight: 700; font-family: inherit; cursor: pointer; letter-spacing: 0.05em; transition: background 0.15s, color 0.15s; }
  .mode-btn.active { background: var(--accent); color: #fff; }
  .mode-btn:not(:last-child) { border-right: 1px solid var(--border-light); }
  .speed-display { font-size: 11px; color: #333333; letter-spacing: 0.05em; }
  .speed-display span { color: #000000; font-weight: 700; font-variant-numeric: tabular-nums; }
  .status-msg { margin-left: auto; font-size: 11px; color: #333333; letter-spacing: 0.06em; font-weight: 600; }
  .table-wrap { overflow: auto; max-height: calc(100vh - 120px); }
  table { border-collapse: collapse; white-space: nowrap; font-size: 11.5px; }
  thead th { background: #1A2744; border: 1px solid #2A3A6A; padding: 6px 8px; text-align: center; font-weight: 700; font-size: 10px; letter-spacing: 0.06em; color: #FFFFFF; position: sticky; top: 0; z-index: 50; font-variant-numeric: tabular-nums; }
  thead th:first-child { position: sticky; left: 0; z-index: 60; background: #1A2744; min-width: 68px; color: #FFFFFF; font-size: 9px; letter-spacing: 0.1em; }
  th.dz-col { background: #888888 !important; color: #FFFFFF !important; border-color: #666666 !important; font-size: 9px; letter-spacing: 0.08em; text-transform: uppercase; min-width: 90px; padding: 4px 8px; vertical-align: middle; }
  th.dz-col span { font-size: 8px; display: block; margin-top: 2px; letter-spacing: 0.05em; }
  td.time-label { position: sticky; left: 0; z-index: 10; background: #1A2744; border: 1px solid #2A3A6A; padding: 4px 10px 4px 8px; font-size: 10.5px; font-weight: 600; color: #FFFFFF; letter-spacing: 0.04em; font-variant-numeric: tabular-nums; text-align: right; }
...
  td.dz { background: #C0C0C0; border-color: #999999; min-width: 90px; color: #000000; }
...
  td.val-neg { background: var(--neg-bg); color: var(--neg-txt); font-weight: 600; }
...
    <select id="aircraft-select">${acOptions}</select>
  </div>
  <div class="control-group">
...
    </div>
  </div>
  <div class="speed-display" id="speed-display">Speed: <span id="speed-val">—</span> km/h</div>
  <div class="status-msg" id="status-msg">SELECT AIRCRAFT TO LOAD TABLE</div>
</div>
<div class="table-wrap">
<table id="calc-table">
  <thead><tr><th>FLIGHT TIME</th>${distHeaders}</tr></thead>
  <tbody id="calc-body"><tr>${timeLabels}</tr></tbody>
</table>
</div>
<div class="footer">
...
    <span style="background:#FFCDD2;color:#B71C1C;padding:2px 8px;border-radius:3px;font-size:10px;font-weight:700;">NEGATIVE C/F</span>
    <span style="background:#FF8080;color:#000;padding:2px 8px;border-radius:3px;font-size:10px;font-weight:700;">CI &gt; 200 (X)</span>
    <span style="background:#BBBBBB;color:#000;padding:2px 8px;border-radius:3px;font-size:10px;font-weight:700;">DEAD ZONE</span>
  </div>
...
  if (val < 0) return 'val-neg'; // NEGATIVE C/F
...
  useEffect(()=>{const svg=svgRef.current;if(!svg)return;const ts=e=>{e.preventDefault();if(e.touches.length===1){drag.current={active:true,x:e.touches[0].clientX,y:e.touches[0].clientY,yp:lv.current.yP,xp:lv.current.xP};}else if(e.touches.length===2){drag.current.active=false;pinchDist.current=Math.hypot(e.touches[1].clientX-e.touches[0].clientX,e.touches[1].clientY-e.touches[0].clientY);}};const tm=e=>{e.preventDefault();const rect=svg.getBoundingClientRect();const svgH=rect.height||280,svgW=rect.width||1200;if(e.touches.length===1&&drag.current.active){const dx=e.touches[0].clientX-drag.current.x;const dy=e.touches[0].clientY-drag.current.y;setYP(drag.current.yp-(dy/svgH)*lv.current.yRZ);const xd=lv.current.days/lv.current.xZ;setXP(Math.max(0,Math.min(drag.current.xp-(dx/svgW)*xd,lv.current.days-xd)));}else if(e.touches.length===2&&pinchDist.current!=null){const d=Math.hypot(e.touches[1].clientX-e.touches[0].clientX,e.touches[1].clientY-e.touches[0].clientY);const f=d/pinchDist.current;setYZ(z=>Math.max(1,Math.min(30,z*f)));setXZ(z=>Math.max(1,Math.min(30,z*f)));pinchDist.current=d;}};const te=e=>{if(e.touches.length===0)drag.current.active=false;if(e.touches.length<2)pinchDist.current=null;};svg.addEventListener('touchstart',ts,{passive:false});svg.addEventListener('touchmove',tm,{passive:false});svg.addEventListener('touchend',te);return()=>{svg.removeEventListener('touchstart',ts);svg.removeEventListener('touchmove',tm);svg.removeEventListener('touchend',te);};},[]);
  useEffect(()=>{const handler=e=>{const svg=svgRef.current;if(!svg)return;const r=svg.getBoundingClientRect();if(e.clientX<r.left||e.clientX>r.right||e.clientY<r.top||e.clientY>r.bottom)return;e.preventDefault();const f=e.deltaY<0?1.25:0.8;setYZ(z=>Math.max(1,Math.min(30,z*f)));setXZ(z=>Math.max(1,Math.min(30,z*f)));};window.addEventListener('wheel',handler,{passive:false});return()=>window.removeEventListener('wheel',handler);},[]);
