...
.savings-panel{padding:16px;display:flex;gap:20px;flex-wrap:wrap;align-items:flex-start}
...
.savings-note{font-size:11px;color:#3A6080;margin-top:12px;line-height:1.6}
.cal-wrap{overflow-x:auto}
table{width:100%;border-collapse:collapse;font-size:12px}
thead th{background:#04101E;color:#3A6080;font-size:9px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;padding:8px 10px;text-align:left;border-bottom:1px solid #0A1E30;position:sticky;top:0;z-index:10}
...
.rating-dot{display:inline-block;width:8px;height:8px;border-radius:50%;margin-right:6px}
.dot-green{background:#00D68F}
.dot-amber{background:#F9A825}
.dot-red{background:#E74C3C}
.forced-buy{display:inline-flex;align-items:center;gap:4px;font-size:10px;color:#F9A825;font-weight:600}
...
async function init(){
  ...
  try{
    const[fpRes,prRes]=await Promise.all([
      fetch('/api/fuel-path'),
      fetch('/api/fuel-profile?discord_id='+DID+'&token='+encodeURIComponent(TOKEN))
    ]);
    const fuelPath=await fpRes.json();
    const prData=await prRes.json();
    loader.style.display='none';
    if(!fuelPath||!fuelPath.days){
      app.innerHTML='<div class="error-msg">Fuel path data unavailable.</div>';
      return;
    }
    const profile=prData.found?prData.profile:null;
    renderDashboard(fuelPath,profile);
  }catch(e){
    loader.style.display='none';
...
function renderDashboard(fp,profile){
  ...
  let html='';
  html+=renderTodayPanel(today,todayUTC,curHour,curMin,fp);
  if(profile){
    html+=renderSavings(fp,profile);
  }else{
  ...
  html+=renderCalendar(fp,todayUTC);
...
function renderTodayPanel(today,todayUTC,curHour,curMin,fp){
  let html='<div class="section"><div class="sec-title">TODAY &mdash; '+todayUTC+' '+MONTHS[fp.month-1]+' '+fp.year+'</div>';
  ...
  if(nextFuel){
    ...
    html+='<div class="today-price '+priceClass+'">$'+nextFuel.price+'</div>';
    html+='<div class="today-slot">Day '+nextFuel.day+' at '+nextFuel.slot+' UTC</div>';
  ...
  if(nextCo2){
    ...
    html+='<div class="today-price '+priceClass+'">$'+nextCo2.price+'</div>';
    html+='<div class="today-slot">Day '+nextCo2.day+' at '+nextCo2.slot+' UTC</div>';
...
function renderSavings(fp,profile){
  const fleetArr=profile.fleet||[];
  const fleetCount=fleetArr.length>0?fleetArr.reduce((s,f)=>s+(f.count||0),0):(profile.aircraft_count||0);
  const tankCap=Number(profile.fuel_tank_capacity)||880597000;
  const dailyBurn=Number(profile.daily_fuel_burn)||0;
  const REV_PER_TYPE={concorde:203939,a380:0,a380f:2422858,b747f:0,a380lh:0,a330:0};
  let dailyRev=Number(profile.daily_revenue)||0;
  if(!dailyRev&&fleetArr.length){
    dailyRev=fleetArr.reduce((s,f)=>{const k=String(f.type||'').toLowerCase();return s+(REV_PER_TYPE[k]||0)*(f.count||0);},0);
  }
  ...
  const totalRandomPrice=fp.days.length*avgRandomPrice;
  const burnLbs=dailyBurn>0?dailyBurn:tankCap*0.3;
  const avgOptPerDay=totalOptimalPrice/fp.days.length/1000;
  const avgRndPerDay=totalRandomPrice/fp.days.length/1000;
  ...
  const fmtDollars=n=>n>=1e6?'$'+(n/1e6).toFixed(2)+'M':n>=1e3?'$'+(n/1e3).toFixed(1)+'K':'$'+Math.round(n);
  let html='<div class="section"><div class="sec-title">YOUR MONTHLY SAVING</div>';
  html+='<div class="savings-panel">';
  html+='<div><div class="savings-num">'+fmtDollars(dollarSaving)+'</div><div class="savings-label">DOLLAR SAVING vs RANDOM BUYING</div></div>';
  if(dailyRev>0){
    html+='<div><div class="savings-num">'+flyingDays.toFixed(1)+' DAYS</div><div class="savings-label">EQUIVALENT FLEET FLYING DAYS</div></div>';
  }
  html+='</div>';
  html+='<div class="savings-note">Based on your fleet of '+(fleetCount||'?')+' aircraft, buying optimally vs randomly across '+fp.days.length+' days.</div>';
  html+='</div>';
  return html;
}

function renderCalendar(fp,todayUTC){
  let html='<div class="section"><div class="sec-title">STEPPING STONE PATH &mdash; FULL MONTH</div>';
  html+='<div class="cal-wrap"><table><thead><tr>';
  html+='<th>DATE</th><th>FUEL SLOT</th><th>FUEL PRICE</th><th></th><th>CO2 SLOT</th><th>CO2 PRICE</th><th></th><th>NOTE</th>';
  html+='</tr></thead><tbody>';
  for(const day of fp.days){
    ...
    html+='<td style="font-weight:700">'+(isToday?'&#9670; ':'')+day.day+' '+MONTHS[fp.month-1]+'</td>';
    if(day.fuel){
      const fRating=day.fuel.rating;
      const isForcedFuel=fRating==='red'||fRating==='amber';
      html+='<td>'+day.fuel.slot+' UTC</td>';
      html+='<td class="price-cell">$'+day.fuel.price+'</td>';
      html+='<td><span class="rating-dot dot-'+fRating+'"></span></td>';
      if(day.co2){
        const cRating=day.co2.rating;
        const isForcedCo2=cRating==='red';
        html+='<td>'+day.co2.slot+' UTC</td>';
        html+='<td class="price-cell">$'+day.co2.price+'</td>';
        html+='<td><span class="rating-dot dot-'+cRating+'"></span></td>';
        if(isForcedFuel&&fRating==='red'){
          html+='<td><span class="forced-buy"><span class="forced-icon">&#9888;</span>FORCED BUY &mdash; LEAST DAMAGING OPTION</span></td>';
        }else{
          html+='<td></td>';
        }
      }else{
      ...
      html+='<td colspan="6" style="color:#3A6080">No schedule data</td><td></td>';
    }
    html+='</tr>';
  }
