// ═══════════════════════════════════════════════════════════════════════════
// BEAGLE GLOBAL — ALLIANCE PROJECTIONS SERVICE — v17
...
<title>Beagle Global — Alliance Projections v17</title>
...
function App(){
  ...
  const W=1200,H=300,ml=70,mr=10,mt=20,mb=36,cw=W-ml-mr,ch=H-mt-mb;
  ...
  return(<div style={{background:"#030B17",minHeight:"100vh",display:"flex",flexDirection:"column"}}>
    ...
    <div style={{padding:"2px 6px 0",flexShrink:0}}>
      <svg viewBox={"0 0 "+W+" "+H} style={{width:"100%",maxHeight:"42vh",display:"block"}}>
        <defs>
        ...
        {xTicks.map(d=>(<g key={d}>
          ...
          <text x={xs(d)} y={mt+ch+16} textAnchor="middle" fill="#4A7090" fontSize="14" fontWeight="600">{xLabels[d]}</text>
        </g>))}
        <g clipPath="url(#cc)">
    ...
    {/* ── RANKING PANEL ── */}
    {showRank&&ranking.length>0&&(<div style={{margin:"4px 10px",background:"#050D1A",border:"1px solid #0A1E30",borderTop:"2px solid #C4920A",borderRadius:4,padding:"10px 14px",flex:1}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8,flexWrap:"wrap",gap:8}}>
        <div>
          ...
          <span style={{fontSize:11,color:"#5A8AAB",marginLeft:10}}>— click any row to inspect</span>
        </div>
      ...
      {/* Two-column ranking grid — all 20 always visible */}
    ...
    {/* ── DETAIL PANEL (shown when a ranking row is clicked) ── */}
