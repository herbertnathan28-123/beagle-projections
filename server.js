...
// ═══════════════════════════════════════════════════════════════════════════
// BEAGLE GLOBAL — ALLIANCE PROJECTIONS SERVICE — v26
...
<title>Beagle Global \u2014 Alliance Projections v26</title>
...
.svg-chart{cursor:grab;user-select:none;touch-action:none;-webkit-user-select:none}
...
  // Interaction values ref — keeps touch handlers current without re-registering
  const iv=useRef({yZ:1,yP:0,xZ:1,xP:0,days:182.6,yRZ:0});
  useEffect(()=>{iv.current={yZ,yP,xZ,xP,days:0,yRZ:0};},[yZ,yP,xZ,xP]);
...
  // Update iv with latest computed values
  useEffect(()=>{iv.current={...iv.current,days,yRZ};},[days,yRZ]);
...
        drag.current={active:true,x:e.touches[0].clientX,y:e.touches[0].clientY,yp:iv.current.yP,xp:iv.current.xP};
      } else if(e.touches.length===2){
        drag.current.active=false;
        pinch.current=Math.hypot(e.touches[1].clientX-e.touches[0].clientX,e.touches[1].clientY-e.touches[0].clientY);
      }
    };
    const tm=e=>{
      e.preventDefault();
...
        setYP(drag.current.yp+(dy/svgH)*iv.current.yRZ);
        const xd=iv.current.days/iv.current.xZ;
        setXP(Math.max(0,Math.min(drag.current.xp-(dx/svgW)*xd,iv.current.days-xd)));
...
  // Mouse drag (desktop)
  const md=useCallback(e=>{
    drag.current={active:true,x:e.clientX,y:e.clientY,yp:yP,xp:xP};
...
    setYP(drag.current.yp+(dy/svgH)*yRZ);
    const xd=days/xZ;
    setXP(Math.max(0,Math.min(drag.current.xp-(dx/svgW)*xd,days-xd)));
  },[yRZ,days,xZ]);
  const mu=useCallback(()=>{drag.current.active=false;svgRef.current?.classList.remove('dragging');},[]);
  useEffect(()=>{
...
      <span style={{fontSize:17,fontWeight:700,color:a.noPace?'#4A7090':c,minWidth:32}}>#{a.projRank}</span>
      <span style={{fontSize:17,color:isB?c:isAct?'#E2EAF4':a.noPace?'#4A7090':'#9AAABB',flex:1,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',fontWeight:isAct?600:400}}>{a.name}</span>
      <span style={{fontSize:15,fontWeight:600,minWidth:26,textAlign:'right',color:a.noPace?'#4A7090':chg>0?'#00E676':chg<0?'#E74C3C':'#4A7090'}}>{a.noPace?'?':chg>0?'\u25b2'+chg:chg<0?'\u25bc'+Math.abs(chg):isB?'\u2605':'\u2014'}</span>
    </div>);
  };
...
      <div>
        <div style={{fontSize:22,fontWeight:700,color:'#E8B84B',letterSpacing:2}}>&#9672; BEAGLE GLOBAL \u2014 ALLIANCE PROJECTIONS</div>
        <div style={{fontSize:14,color:'#8AAABB',marginTop:3,letterSpacing:1}}>
...
        <div style={{fontSize:24,fontWeight:700,color:'#E8B84B'}}>{'$'+BS.toLocaleString('en',{minimumFractionDigits:2})+'M'}</div>
        <div style={{fontSize:14,color:'#8AAABB',letterSpacing:1}}>{BP.toFixed(3)} $M/DAY</div>
      </div>
    </div>
...
      <svg ref={svgRef} className="svg-chart" viewBox={'0 0 '+W+' '+H} style={{width:'100%',maxHeight:'52vh',display:'block'}} onMouseDown={md}>
        <defs>
...
        <div>
          <span style={{fontSize:15,color:'#8AAABB',letterSpacing:1}}>PROJECTED RANKING AT </span>
          <span style={{fontSize:15,color:'#E8B84B',fontWeight:700,letterSpacing:1}}>{pk}</span>
          <span style={{fontSize:13,color:'#4A7090',marginLeft:10}}>\u2014 tap to toggle line</span>
        </div>
        <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
          <span style={{fontSize:14,color:'#8AAABB'}}>Beagle</span>
          <span style={{fontSize:17,fontWeight:700,color:'#E8B84B'}}>#{beagle.rank}</span>
          <span style={{fontSize:16,color:'#5A8AAB'}}>\u2192</span>
          <span style={{fontSize:20,fontWeight:700,color:rChg>0?'#00E676':'#E8B84B'}}>#{bRank?.projRank}</span>
          {rChg>0&&<span style={{fontSize:16,fontWeight:700,color:'#00E676'}}>\u25b2{rChg}</span>}
          {rChg<0&&<span style={{fontSize:16,fontWeight:700,color:'#E74C3C'}}>\u25bc{Math.abs(rChg)}</span>}
          {selA&&!selA.isBeagle&&(()=>{
