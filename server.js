...
function logVisit(req) {
  ...
  if (visitorLog.length > 200) visitorLog.pop(); // keep last 200
...
function App(){
  ...
  const bb={borderRadius:3,fontSize:22,fontWeight:700,cursor:"pointer",letterSpacing:1,fontFamily:"inherit",padding:"5px 12px"};
  ...
  if(loading)return(<div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",flexDirection:"column",gap:16}}><div style={{fontSize:35,color:"#E8B84B"}}>◈</div><div style={{fontSize:25,color:"#2C4A68",letterSpacing:2}}>LOADING PROJECTIONS...</div></div>);
  ...
  const renderRow=a=>{
    ...
    return(<div key={a.name} style={{display:"flex",alignItems:"center",gap:6,padding:"6px 8px",background:isB?"#1A1000":"transparent",borderRadius:3,border:isB?"1px solid #C4920A30":"1px solid transparent"}}>
      <span style={{fontSize:20,fontWeight:700,color:a.noPace?"#3A6090":c,minWidth:22}}>#{a.projRank}</span>
      <span style={{fontSize:20,color:isB?c:a.noPace?"#2C4A68":"#8AAABB",flex:1,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{a.name}</span>
      <span style={{fontSize:19,fontWeight:600,minWidth:20,textAlign:"right",color:a.noPace?"#2C4A68":change>0?"#00E676":change<0?"#E74C3C":"#2C4A68"}}>{a.noPace?"?":change>0?"▲"+change:change<0?"▼"+Math.abs(change):isB?"★":"—"}</span>
    </div>);
  };

  return(<div style={{background:"#030B17",minHeight:"100vh",display:"flex",flexDirection:"column"}}>
    <div style={{background:"linear-gradient(90deg,#04101E,#0A1C32)",borderBottom:"2px solid #C4920A",padding:"14px 18px",display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:6}}>
      <div>
        <div style={{fontSize:26,fontWeight:700,color:"#E8B84B",letterSpacing:2}}>◈ BEAGLE GLOBAL — ALLIANCE PROJECTIONS</div>
        <div style={{fontSize:21,color:"#2C4A68",marginTop:2,letterSpacing:1}}>
      ...
      <div style={{textAlign:"right"}}>
        <div style={{fontSize:28,fontWeight:700,color:"#E8B84B"}}>{"$"+B_SV.toLocaleString("en",{minimumFractionDigits:2})+"M"}</div>
        <div style={{fontSize:21,color:"#2C4A68",letterSpacing:1}}>{B_PACE.toFixed(3)} $M/DAY</div>
      </div>
    </div>

    <div style={{display:"flex",gap:6,padding:"10px 18px",background:"#040C18",borderBottom:"1px solid #0A1E30",alignItems:"center",flexWrap:"wrap"}}>
      <span style={{fontSize:21,color:"#1E3A55",letterSpacing:1,marginRight:4}}>PERIOD</span>
      ...
      <div className="legend-row" style={{marginLeft:"auto",display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
        {[["#E8B84B","Beagle"],["#00E676","< 100d"],["#69F0AE","catching"],["#3A6090","away"],["#E74C3C","passed"]].map(([c,l])=>(
          <span key={l} style={{display:"flex",alignItems:"center",gap:4}}>
            ...
            <span style={{fontSize:19.5,color:"#2C4A68"}}>{l}</span>
          </span>
        ))}
      </div>
    </div>

    <div style={{padding:"2px 6px 0",flexShrink:0,maxHeight:"32vh",overflow:"hidden"}}>
      <svg viewBox={"0 0 "+W+" "+H} style={{width:"100%",maxHeight:"32vh",display:"block"}}>
        <defs>
        ...
        {yTicks.map(v=>(<g key={v}>
          ...
          <text x={ml-6} y={ys(v)+3.5} textAnchor="end" fill="#182838" fontSize="19.5">{v>=1000?(v/1000).toFixed(1)+"k":v}</text>
        </g>))}
        {xTicks.map(d=>(<g key={d}>
          <line x1={xs(d)} x2={xs(d)} y1={mt} y2={mt+ch} stroke="#1E3A5F" strokeWidth="0.6" strokeDasharray="4,6"/>
          <text x={xs(d)} y={mt+ch+14} textAnchor="middle" fill="#182838" fontSize="19" fontWeight="600">{xLabels[d]}</text>
        </g>))}
        <g clipPath="url(#cc)">
          ...
          {selA&&!selA.isBeagle&&(()=>{const c=col(selA),cd=crossDay(selA);return(<g>
            ...
            {cd&&(<g>
              ...
              <text x={xs(cd)} y={ys(B_SV+B_PACE*cd)-46} textAnchor="middle" fill={c} fontSize="20.5" fontWeight="700">{fmtDate(selA.overtakeDate)}</text>
            </g>)}
          </g>);})()}
        ...
        <text x={ml-6} y={ys(B_SV)+3.5} textAnchor="end" fill="#E8B84B" fontSize="20" fontWeight="700">★{beagle.rank}</text>
        ...
        <text x="10" y={mt+ch/2} textAnchor="middle" fill="#162030" fontSize="18.5" fontWeight="600" letterSpacing="0.8" transform={"rotate(-90,10,"+(mt+ch/2)+")"}>SV ($M)</text>
      </svg>
    </div>

    {showRank&&ranking.length>0&&(<div style={{margin:"4px 10px",background:"#050D1A",border:"1px solid #0A1E30",borderTop:"2px solid #C4920A",borderRadius:4,padding:"14px 18px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8,flexWrap:"wrap",gap:8}}>
        <div>
          <span style={{fontSize:21,color:"#2C4A68",letterSpacing:1}}>PROJECTED RANKING AT </span>
          <span style={{fontSize:21,color:"#E8B84B",fontWeight:700,letterSpacing:1}}>{period} MONTH{period>1?"S":""}</span>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          <span style={{fontSize:20,color:"#2C4A68"}}>Beagle</span>
          <span style={{fontSize:24,fontWeight:700,color:"#E8B84B"}}>#{beagle.rank}</span>
          <span style={{fontSize:25,color:"#2C4A68"}}>→</span>
          <span style={{fontSize:29,fontWeight:700,color:rankChange>0?"#00E676":"#E8B84B"}}>#{beagleRank?.projRank}</span>
          {rankChange>0&&<span style={{fontSize:22,fontWeight:700,color:"#00E676"}}>▲{rankChange}</span>}
          {rankChange<0&&<span style={{fontSize:22,fontWeight:700,color:"#E74C3C"}}>▼{Math.abs(rankChange)}</span>}
          {selA&&!selA.isBeagle&&(()=>{
            ...
            return(<span style={{display:"flex",alignItems:"center",gap:6,marginLeft:12,paddingLeft:12,borderLeft:"1px solid #162030"}}>
              <span style={{fontSize:20,color:c,fontWeight:600}}>{selA.name}</span>
              <span style={{fontSize:24,fontWeight:700,color:c}}>#{selA.rank}</span>
              <span style={{fontSize:25,color:"#2C4A68"}}>→</span>
              <span style={{fontSize:27,fontWeight:700,color:sc>0?"#00E676":sc<0?"#E74C3C":c}}>#{sp?.projRank}</span>
              {sc!==0&&<span style={{fontSize:22,fontWeight:700,color:sc>0?"#00E676":"#E74C3C"}}>{sc>0?"▲"+sc:"▼"+Math.abs(sc)}</span>}
            </span>);
          })()}
        </div>
      </div>
      <div className="rank-grid" style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:"0 20px"}}>
        <div><div style={{fontSize:19,color:"#182838",fontWeight:600,letterSpacing:1,padding:"0 6px 4px"}}>1 — 10</div>{ranking.slice(0,10).map(renderRow)}</div>
        <div><div style={{fontSize:19,color:"#182838",fontWeight:600,letterSpacing:1,padding:"0 6px 4px"}}>11 — 20</div>{ranking.slice(10,20).map(renderRow)}</div>
      </div>
    </div>)}

    {selA&&!selA.isBeagle&&(<div style={{margin:"4px 10px 4px",background:"#050D1A",border:"1px solid "+col(selA)+"30",borderLeft:"3px solid "+col(selA),borderRadius:4,padding:"14px 18px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
        <div>
          <div style={{fontSize:25,fontWeight:700,color:"#E2EAF4"}}>#{selA.rank} {selA.name}</div>
          <div style={{fontSize:21,color:"#2C4A68",marginTop:2}}>{"SV $"+selA.sv.toLocaleString("en",{minimumFractionDigits:2})+"M · Pace"} {selA.pace?.toFixed(3)??"—"}/day · Gap {selA.gap>0?"+":""}{selA.gap.toFixed(0)}M</div>
        </div>
        {selA.catchable?(<div style={{background:"#04120A",border:"1px solid "+col(selA),borderRadius:4,padding:"4px 10px",textAlign:"right"}}>
          <div style={{fontSize:20,color:col(selA),letterSpacing:1}}>BEAGLE OVERTAKES</div>
          <div style={{fontSize:24,fontWeight:700,color:col(selA)}}>{fmtDate(selA.overtakeDate)}</div>
          <div style={{fontSize:20,color:"#2C4A68"}}>in {Math.round(selA.daysTo)} days</div>
        </div>):selA.passed?(<div style={{background:"#1C0606",border:"1px solid #E74C3C",borderRadius:4,padding:"4px 10px",textAlign:"right"}}>
          <div style={{fontSize:20,color:"#E74C3C",letterSpacing:1}}>STATUS</div>
          <div style={{fontSize:24,fontWeight:700,color:"#E74C3C"}}>✓ OVERTAKEN</div>
        </div>):(<div style={{background:"#06101C",border:"1px solid #3A6090",borderRadius:4,padding:"4px 10px",textAlign:"right"}}>
          <div style={{fontSize:20,color:"#3A6090",letterSpacing:1}}>STATUS</div>
          <div style={{fontSize:23,fontWeight:700,color:"#3A6090"}}>PULLING AWAY</div>
        </div>)}
      </div>
      <div style={{display:"flex",gap:5,marginBottom:8}}>
        {detailProj.map(({mo,sv,rank})=>{const ahead=sv-(B_SV+B_PACE*MTD[mo]),c=ahead<=0?"#00E676":ahead<100?"#69F0AE":"#3A6090";return(<div key={mo} style={{flex:1,background:"#030B17",borderRadius:4,padding:"7px 5px",textAlign:"center",border:"1px solid #0A1E30"}}>
          <div style={{fontSize:20,color:"#182838",fontWeight:600,letterSpacing:1,marginBottom:3}}>{mo}MO</div>
          <div style={{fontSize:22,fontWeight:700,color:"#C0CCD8"}}>{"$"+sv.toFixed(0)+"M"}</div>
          <div style={{fontSize:19.5,color:c,marginTop:2,fontWeight:600}}>{ahead<=0?"✓ BEHIND":"+$"+ahead.toFixed(0)+"M ahead"}</div>
          <div style={{fontSize:19,color:"#182838",marginTop:1}}>Beagle #{rank}</div>
        </div>);})}
      </div>
      <div className="stat-row" style={{display:"flex",alignItems:"center",gap:20,borderTop:"1px solid #0A1E30",paddingTop:8,flexWrap:"wrap"}}>
        <div><div style={{fontSize:20,color:"#182838",letterSpacing:1}}>DAILY CLOSURE</div><div style={{fontSize:23,fontWeight:700,color:selA.catchable?col(selA):"#3A6090"}}>{selA.closure!=null?(selA.closure>0?"+":"")+"$"+selA.closure.toFixed(3)+"M/day":"unknown"}</div></div>
        <div><div style={{fontSize:20,color:"#182838",letterSpacing:1}}>THEIR PACE</div><div style={{fontSize:23,fontWeight:700,color:"#E2EAF4"}}>{selA.pace?.toFixed(3)??"—"} $M/day</div></div>
        <div><div style={{fontSize:20,color:"#182838",letterSpacing:1}}>BEAGLE PACE</div><div style={{fontSize:23,fontWeight:700,color:"#E8B84B"}}>{B_PACE.toFixed(3)} $M/day</div></div>
        <div style={{marginLeft:"auto",fontSize:22,fontWeight:700,color:selA.catchable?"#69F0AE":selA.passed?"#00E676":"#3A6090"}}>{selA.catchable?"↓ CONVERGING":selA.passed?"✓ CLEAR":"↑ DIVERGING"}</div>
      </div>
    </div>)}

    <div style={{padding:"5px 10px 16px",overflowY:"auto",maxHeight:500}}>
      <div style={{fontSize:20,color:"#162030",letterSpacing:1,marginBottom:6,paddingLeft:2}}>{fullField?"ALL ALLIANCES":"CLOSE PACK"}{!fullField&&<span style={{color:"#1E3A55"}}> · {FAR.length} far alliances hidden</span>}</div>
      {[...(fullField?[...CLOSE,...FAR]:CLOSE)].sort((a,b)=>a.rank-b.rank).map(a=>{
        ...
        return(<div key={a.name} onClick={()=>setSelected(isSel?null:a.name)} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 16px",marginBottom:4,background:isSel?c+"14":"#040C18",border:"1px solid "+(isSel?c:"#0A1E30"),borderLeft:"3px solid "+c,borderRadius:4,cursor:"pointer",transition:"all 0.12s"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <span style={{fontSize:22,fontWeight:700,color:c,minWidth:26}}>#{a.rank}</span>
            <span style={{fontSize:22,fontWeight:isSel?700:400,color:isSel?c:"#C0CCD8"}}>{a.name}</span>
            {a.passed&&<span style={{fontSize:20,color:"#00E676",fontWeight:700}}>✓</span>}
          </div>
