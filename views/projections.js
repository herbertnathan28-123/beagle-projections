...
function App(){
  ...
  const[teamR,setTeamR]=useState(null);
  useEffect(()=>{fetch('/api/team-rating').then(r=>r.json()).then(d=>setTeamR(d)).catch(()=>{});},[]);
  ...
  return(<div style={{background:'#030B17',height:'100%',minHeight:'100vh',display:'flex',flexDirection:'column'}}>
    {teamR&&teamR.overall!=null&&(
      <div style={{background:'#C4920A',color:'#030B17',padding:'7px 16px',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:6,fontWeight:700,letterSpacing:1,fontSize:sz(12,15,0.008)}}>
        <span>PREVIOUS 7 DAYS TEAM RANKING · {teamR.history.map(h=>h.rating==null?'—':h.rating).join(', ')}</span>
        <span>OVERALL {teamR.overall}</span>
      </div>
    )}
