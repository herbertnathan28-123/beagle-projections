...
function App(){
  const[apiData,setApiData]=useState(null);const[loading,setLoading]=useState(true);const[pk,setPk]=useState('6MO');const[mode,setMode]=useState('SV');const[act,setAct]=useState(new Set());const[teamRating,setTeamRating]=useState(null);
  ...
  useEffect(()=>{fetch('/api/team-rating').then(r=>r.json()).then(d=>setTeamRating(d)).catch(()=>{});},[]);
  ...
  return(<div style={{background:'#030B17',height:'100%',minHeight:'100vh',display:'flex',flexDirection:'column'}}>
    {teamRating&&teamRating.overall!=null&&(
      <div style={{background:'#C4920A',color:'#030B17',padding:'6px 16px',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:6,fontWeight:700,letterSpacing:1,fontSize:sz(11,14,0.008)}}>
