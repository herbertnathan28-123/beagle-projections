...
function App(){
  ...
  useEffect(()=>{fetch('/api/player-stats').then(r=>r.ok?r.json():null).then(d=>{if(d&&d.teamRating)setTeamRating(d.teamRating);}).catch(()=>{});},[]);
  ...
  return(<div style={{background:'#030B17',height:'100%',minHeight:'100vh',display:'flex',flexDirection:'column'}}>
    {teamRating&&teamRating.overall!=null&&(
      <div style={{background:'#C4920A',color:'#030B17',padding:'6px 16px',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:6,fontWeight:700,letterSpacing:1,fontSize:13}}>
